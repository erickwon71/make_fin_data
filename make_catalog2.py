...
def get_gspread_sheet():
    ...
    spreadsheet_url = 'https://docs.google.com/spreadsheets/d/14hG82qYt8858f6yIR553iGIlkqgaywnD9k1GeICmoSk/edit#gid=1950763448'

    # 스프레스시트 문서 가져오기 
    doc = gc.open_by_url(spreadsheet_url)

    # 시트 선택하기
    worksheet = doc.worksheet('스크래핑')
    
    return worksheet

def read_gspread_cell(cell_name):
    ws = get_gspread_sheet()
    
    cell_data = ws.acell(cell_name).value
    return cell_data
    
   
def write_gspread_cell(cell_name, corp_name):    
    ret_val = True
    ws = get_gspread_sheet()
    ...
    try_cnt = 10
    while(1):
        cell_data = ws.acell('A11').value
        if (cell_data == '로드 중...') or (cell_data == 'Loading...'):
            #print(cell_data)
            #time.sleep(0.1)
            try_cnt -= 1
            if try_cnt == 0:
...
if __name__=="__main__":
    ...
    for i in range(len(dfcodes)):
        ...
        if write_gspread_cell('B4', dfcodes.iloc[i, 1]) == False:
            wb.save(fname)
            continue
        ...
        #print(corp_tbl[0])
        try:
            print('', corp_tbl[0].iloc[1, 1], end='')
            sheet.cell(row=rown, column=31, value=corp_tbl[0].iloc[1, 1]) #제품
        except:
            print('', 'NaN', end='')        
